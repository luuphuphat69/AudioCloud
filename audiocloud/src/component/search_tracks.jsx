...
const Search_Tracks = ({ searchResults }) => {
    ...
    const updatePlays = async (audioId) => {
        try {
            await axios.put(`http://3.86.36.56:8000/v1/audio/update-plays/${audioId}`);
        } catch (error) {
            console.log(error);
        }
    }
    const updateHistory = async (audioId) => {
        try {
            await axios.put(`http://3.86.36.56:8000/v1/history/update-history/${audioId}/${userId}`);
        } catch (err) {
            console.log(err);
        }
    }
                ...
                {paginatedData.map((item) => (
                    <div class="d-block d-md-flex podcast-entry mb-5" style={{ backgroundColor: "#EDEDED" }}>
                        <div className="image-container mt-4">
                            {item.PhotoURL ? (
                                <img src={item.PhotoURL} style={{ width: '100%', height: '160px' }} alt="" />
                            ) : (
                                <img style={{ width: '100%', height: '160px' }} src="./src/assets/img/blur_img.png" alt="Default" />
                            )}
                        ...
                        <div class="text">
                            ...
                            <p className='ml-3'>Lượt nghe: {item.Plays}</p>
                            <div className="ml-3" style={{ display: 'flex' }}>
                                <button className="mr-3 mb-3" onClick={() => handleLike(item.AudioId)}>
                                    <div className="box" style={{ display: 'flex', alignItems: 'center' }}>
                                        <img className="horizontal-button" src="../src/assets/img/icon/heart.png" alt="Heart Icon" style={{ width: '25px', height: '25px' }} />
                                    </div>
                                </button>

                                <button className="mr-3 mb-3 custom-button" onClick={handlePlaylistClick}>
                                    <div className="box" style={{ display: 'flex', alignItems: 'center' }}>
                                        <img className="horizontal-button" src="../src/assets/img/icon/playlist.png" alt="Playlist Icon" style={{ width: '25px', height: '25px' }} />
                                    </div>
                                </button>

                                <button className="mr-3 mb-3 custom-button" onClick={() => handleDownload(item.AudioURL, item.AudioName)}>
                                    <div className="box" style={{ display: 'flex', alignItems: 'center' }}>
                                        <img className="horizontal-button" src="../src/assets/img/icon/download.png" alt="Download Icon" style={{ width: '25px', height: '25px' }} />
                                    </div>
                                </button>
                            </div>
