...
const EditPlaylist_Popup = ({ playlistId, closePopup }) => {
    ...
    const handleEdit = async (playlistId) => {
        try {
            ...
            await axios.put(`http://3.106.53.25:8000/v1/playlist/edit/${playlistId}`, formData, {
                method: "PUT",
                credentials: 'include',
                headers: {
    ...
    const handleRemoveAudio = async (audioId, playlistId) => {
        await axios.put(`http://3.106.53.25:8000/v1/playlist/remove-audio/${audioId}/${playlistId}`);
        setRemoveNotification(true);
        const updatedResponse = await axios.get(`http://3.106.53.25:8000/v1/playlist/getInfo/${playlistId}`);
