...
const PopupForm = ({ closePopup }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://54.206.75.221:8000/v1/user/get-info/${userId}`, { withCredentials: true });
  ...
  useEffect(() => {
    const fetchToken = async () => {
      try {
        const response = await axios.get('http://54.206.75.221:8000/getcookie', { withCredentials: true });
  ...
  const handleChanges = async () => {
    ...
    axios.put(`http://54.206.75.221:8000/v1/user/edit/${userId}`, formData, {
      method: "PUT",
      credentials: "include",
      headers: {
