...
const Playlists = () => {
  ...
  useEffect(() => {
    const fetchToken = async () => {
      try {
        ...
        const responseData = await axios.get(`http://3.86.36.56:8000/v1/playlist/get-user-playlist/${_user?.userId}`);
  ...
  const handleRemove = async (playlistId) => {
    ...
    if (shouldDelete) {
      await axios.delete(`http://3.86.36.56:8000/v1/playlist/delete/${playlistId}`);
