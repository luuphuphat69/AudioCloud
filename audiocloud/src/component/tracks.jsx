  ...
  useEffect(() => {
    const fetchToken = async () => {
      try {
        ...
        const responseData = await axios.get(`http://54.161.251.210:8000/v1/audio/getTracks/${_user?.userId}`);
  ...
  if (isDesktopOrLaptop) {
    ...
    return (
      <section className="cart_area padding_top">
        <div className="container">
          {data?.map((item, index) => (
            <div key={item.AudioId} className="d-block podcast-entry mb-4" style={{ backgroundColor: "#FFA33C" }}>
