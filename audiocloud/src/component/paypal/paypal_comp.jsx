...
const PaymentComponent = ({ userId, type, amount, closePopup }) => {
  ...
  const handleUpdateBilling = async () => {
    try {
      await axios.post(`http://3.86.36.56:8000/v1/billing/create-bill/${userId}`);
    } catch (err) {
      console.log(err);
    }
  }
  const handleUpdatePro = async () => {
    try {
      ...
      if (type === 'Artist') {
        await axios.put(`http://3.86.36.56:8000/v1/user/update-artist/${user.userId}`);
      }
      if (type === 'Listener') {
        await axios.put(`http://3.86.36.56:8000/v1/user/update-pro/${user.userId}`);
      }
    } catch (err) {
      console.log(err);
    }
