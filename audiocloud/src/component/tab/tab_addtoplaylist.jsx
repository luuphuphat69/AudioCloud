...
const Tab_AddToPlaylist = (audioId) => {
    ...
    useEffect(() => {
        if (userId) {
            const fetchData = async () => {
                try {
                    const response = await axios.get(`http://54.206.75.221:8000/v1/playlist/get-user-playlist/${userId}`);
    ...
    const handleClick = async (audioId, playlistId) => {
        ...
        await axios.put(`http://54.206.75.221:8000/v1/playlist/add-to-playlist/${audioId.audioId}/${playlistId}`);
